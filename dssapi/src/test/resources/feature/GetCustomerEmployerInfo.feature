...
Feature: This feature is to test getCustomerIncomeInfo api of TS2 service
  ...
  @tag1
  Scenario Outline: Testing the /v1/getCustomerEmployerapi with valid account details
    Given post operation to request getCustomerEmployerapi api to get the customerIncome information on "<cardNbr>" "<accountId>" "<custId>" and "<operatorId>"
    Then validate the status code as "<statusCode>" and customer employer info is fetched with status "<statusCode>" and desc "<desc>"
