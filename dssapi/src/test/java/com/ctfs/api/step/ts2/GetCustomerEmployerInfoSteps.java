...
import com.ctfs.api.pojos.response.GetCustomerEmployerInfoResPojo;
...
import com.ctfs.api.service.ts2.GetCustomerEmployerInfoService;
...
public class GetCustomerEmployerInfoSteps extends AbstractStep {
	private final Logger log = LoggerFactory.getLogger(GetCustomerEmployerInfoSteps.class);
	
	@Autowired
	private TS2RequestPojo requestObj;
	
	static GetCustomerEmployerInfoResPojo res_obj = null;
    
    @Autowired
    private GetCustomerEmployerInfoService service;
    
    @Autowired
	private StepDefinitionDataManager stepDefinitionDataManager;
    
    @Given("post operation to request getCustomerEmployerapi api to get the customerIncome information on {string} {string} {string} and {string}")
...
//			dpm.initializeTestProfile("group=ApiGeneric");
			service.getCustomerEmpInfo(getPayload(cardNbr,
				 	accountId,
				 	custId,
					op_id));
	...
	@Then("validate the status code as {string} and customer employer info is fetched with status {string} and desc {string}")
	public void validate_getCustomerEmployerInfo(String statusCode,String status, String desc) throws Throwable {
		try {
			Response response = (Response)stepDefinitionDataManager.getStoredObjectMap().get("getCustomerEmployerInfoRes");
			
			Assert.assertTrue(statusCode.equals(String.valueOf(response.getStatusCode())));
//			int code=response.getStatusCode();
			if(response.getStatusCode()==200){
				res_obj = response.getBody().as(GetCustomerEmployerInfoResPojo.class);
