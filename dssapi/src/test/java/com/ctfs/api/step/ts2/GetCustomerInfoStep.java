...
public class GetCustomerInfoStep extends AbstractStep {
	private final Logger log = LoggerFactory.getLogger(GetCustomerInfoStep.class);
	...
	@Autowired
	private GetCustomerInfoService service;
	...
	public void post_operation(String cardNbr, 
		...
		try {
			...
			service.getCustomerInfo(requestObj);
	...
	@Then("validate the status code as {int} and customer info is fetched with status {string} and desc {string}")
	public void validate_getCustomer(int statusCode,String status, String desc) throws Throwable {
		try {
			Response response = (Response)stepDefinitionDataManager.getStoredObjectMap().get("getCustomerInfoRes");
			...
			if(response.getStatusCode()==200){
				...
				if (res_obj.getStatus().equals("000")) {
					...
					Assert.assertNotNull(res_obj.getCustomer());
				}
