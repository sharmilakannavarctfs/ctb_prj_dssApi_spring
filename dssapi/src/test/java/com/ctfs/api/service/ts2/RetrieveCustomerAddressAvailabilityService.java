...
@RetailBankService
public class RetrieveCustomerAddressAvailabilityService extends BaseExecutor {
    protected RetrieveCustomerAddressAvailabilityService(RestService restService, ApplicationProperties applicationProperties) {
        super(restService, applicationProperties, "TS2SERVICE_V1");
    }

    private final Logger log = LoggerFactory.getLogger(RetrieveCustomerAddressAvailabilityService.class);
    ...
    public void postCall(Object requestbody) throws URISyntaxException {
        try {
            ...
            setBody(requestbody);
            Response response = post(Endpoints.ts2_retrieveCustomerAddressAvailability);
            stepDefinitionDataManager.addToStoredObjectMap("retrieveCustomerAddressAvailability", response);
