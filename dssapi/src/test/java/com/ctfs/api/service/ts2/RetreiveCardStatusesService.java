...
@RetailBankService
public class RetreiveCardStatusesService extends BaseExecutor{

	protected RetreiveCardStatusesService(RestService restService, ApplicationProperties applicationProperties) {
		super(restService, applicationProperties,"TS2SERVICE_V1");
		// TODO Auto-generated constructor stub
	}

	private final Logger log = LoggerFactory.getLogger(RetreiveCardStatusesService.class);

	@Autowired
	private StepDefinitionDataManager stepDefinitionDataManager;
	
	public void retrieveCardStatuses(Object requestbody) throws URISyntaxException {
		try {
			System.out.println("Service in body: ");
			log.info("request body: "+ requestbody,true);
			setBody(requestbody);
			Response response = post(Endpoints.ts2_retrieveCardStatuses);
			stepDefinitionDataManager.addToStoredObjectMap("retrievecardstatusesres", response);
