...
@RetailBankService
public class RetrieveMessagesServiceCnt extends BaseExecutor{

	protected RetrieveMessagesServiceCnt(RestService restService, ApplicationProperties applicationProperties) {
		super(restService, applicationProperties,"MESSAGESERVICE_V1");
	}

	private final Logger log = LoggerFactory.getLogger(RetrieveMessagesServiceCnt.class);
	...
	public void post(Object requestbody) throws URISyntaxException {
		try {
			...
			setBody(requestbody);
			Response response = post(Endpoints.msg_retrieveMessagesCnt);
			stepDefinitionDataManager.addToStoredObjectMap("retrieveMessagesresponse", response);
