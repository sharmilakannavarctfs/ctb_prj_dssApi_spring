...
@RetailBankService
public class RetrieveMessagesService extends BaseExecutor{

	protected RetrieveMessagesService(RestService restService, ApplicationProperties applicationProperties) {
		super(restService, applicationProperties,"MESSAGESERVICE_V1");
	}

	private final Logger log = LoggerFactory.getLogger(RetrieveMessagesService.class);
	...
	public void post(Object requestbody) throws URISyntaxException {
		try {
			...
			setBody(requestbody);
			Response response = post(Endpoints.msg_retrieveMessages);
