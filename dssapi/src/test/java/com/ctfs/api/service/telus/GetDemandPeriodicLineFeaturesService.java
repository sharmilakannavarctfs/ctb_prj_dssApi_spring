...
public class GetDemandPeriodicLineFeaturesService extends BaseExecutor {
	...
	private final Logger log = LoggerFactory.getLogger(GetDemandPeriodicLineFeaturesService.class);
	...
	public void getDemandPeriodicLineFeaturesWithToken(Object requestbody, Object token) {
		try {
			setBody(requestbody);
			setHeader(token);
			Response response = post(Endpoints.getDemandPeriodicLineFeatures);
	...
	public void getDemandPeriodicLineFeaturesWithDisablePagination(Object requestbody, Object pagination) {
		try {
			setBody(requestbody);
