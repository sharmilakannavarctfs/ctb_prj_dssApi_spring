...
//                _value, _by.toString()));
	...
	public void scrollIntoView(WebElement element) throws InterruptedException {
		((JavascriptExecutor) driverManager.getWebDriver()).executeScript("arguments[0].scrollIntoView(true);",
				element);
		Thread.sleep(500);
		((JavascriptExecutor) driverManager.getWebDriver()).executeScript("window.scrollBy(0,-100)");
		Thread.sleep(500);
	}
...
//		        }
	...
	public void validateEmptyText(By _by, int waitSeconds) throws ValidationException {
		if (!waitForEmptyText(_by, waitSeconds)) {
			throw new ValidationException(
					"Failed validateEmptyText by=" + _by.toString() + " waitSeconds=" + waitSeconds);
		}
	}
	...
	public void validateNonEmptyText(By _by, int waitSeconds) throws ValidationException {
		if (!waitForNonEmptyText(_by, waitSeconds)) {
			throw new ValidationException(
					"Failed validateNonEmptyText by=" + _by.toString() + " waitSeconds=" + waitSeconds);
		}
	}
